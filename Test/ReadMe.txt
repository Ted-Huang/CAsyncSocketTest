...
應用程式精靈已經為您建立了這個 Test 應用程式。這個應用程式不僅示範了使用 MFC 的基本概念，也是您撰寫應用程式的起點。
...
Test.h
    這是應用程式的主要標頭檔。
    它包含了特定標頭 (包括 Resource.h)，並宣告 CTestApp 應用程式類別。

Test.cpp
    這是含有應用程式類別 CTestApp 的主要應用程式原始程式檔。

Test.rc
    這是程式所用的所有 Microsoft Windows 資源的列表。它含有儲存在 RES 子目錄中的圖示、點陣圖和游標。您可以直接在 Microsoft Visual C++ 內編輯這個檔案。您的專案資源是在 1028。

res\Test.ico
    這是用來做為應用程式圖示的圖示檔。這個圖示是包含在主要資源檔 Test.rc 中。
...
TestDlg.h、TestDlg.cpp - 對話方塊
    這些檔案包含您的 CTestDlg 類別。這個類別會定義應用程式主對話方塊的行為。對話方塊的範本是在可於 Microsoft Visual C++ 編輯的 Test.rc 中。
...
StdAfx.h, StdAfx.cpp
    這些檔案是用來建置名為 Test.pch 的先行編譯標頭 (PCH) 檔，以及名為 StdAfx.obj 的先行編譯型別檔。
